...
export function OrganizationClient() {
  ...
  const [viewMode, setViewMode] = useState<"chart" | "diagram" | "grid" | "table">("chart");
  ...
  return (
    <div className="space-y-12">
      ...
      <div className="flex flex-col lg:flex-row items-stretch lg:items-center justify-between gap-4 p-4 rounded-2xl bg-surface-secondary border border-border/60 shadow-xs">
        <div className="flex flex-wrap items-center gap-1.5">
          <Button
            variant={viewMode === "chart" ? "default" : "ghost"}
            size="sm"
            onClick={() => setViewMode("chart")}
            className="rounded-xl gap-2 font-medium text-xs"
          >
      ...
      {/* VIEW 1: Official Diagram Lightbox Container */}
      {viewMode === "diagram" && (
        <section className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-2xl font-heading font-bold text-foreground">
                Official Organizational Chart Diagram
              </h2>
              <p className="text-xs text-muted-foreground">
                High-resolution corporate structure graphic from the original AnyCargh document.
              </p>
            </div>
            <Button
              variant="outline"
          ...
          <div className="p-4 sm:p-8 rounded-3xl bg-background border border-border/60 shadow-lg overflow-x-auto">
            <div className="min-w-[900px] relative rounded-2xl overflow-hidden border border-border/40 bg-white p-4">
              <Image
                src="/org-chart.png"
                alt="AnyCargh Official Organizational Chart Diagram"
                width={5549}
                height={1050}
      ...
      {/* VIEW 2: Interactive Hierarchy Tree */}
      {viewMode === "chart" && (
        <section className="space-y-8">
          <div className="text-center space-y-2">
            ...
            <p className="text-xs sm:text-sm text-muted-foreground max-w-xl mx-auto">
              Visual reporting flow connecting the CEO to functional managers and operational personnel.
            </p>
          </div>

          <div className="p-6 sm:p-10 rounded-3xl bg-background border border-border/60 shadow-sm overflow-x-auto">
            <div className="min-w-[950px] flex flex-col items-center space-y-8">
              {/* Level 1: CEO */}
              <div className="flex flex-col items-center">
                <div className="relative p-6 rounded-2xl bg-gradient-to-br from-amber-500/10 via-amber-500/5 to-transparent border-2 border-amber-500/50 shadow-xl text-center w-80">
                  <span className="text-[10px] font-extrabold tracking-widest uppercase px-3 py-1 rounded-full bg-amber-500 text-white mb-2 inline-block shadow-xs">
                    Chief Executive Officer
                  </span>
                  <div className="w-14 h-14 rounded-2xl bg-gradient-to-br from-amber-500 to-amber-700 text-white font-bold text-xl flex items-center justify-center mx-auto mb-2 shadow-md border border-white/20">
                    JB
                  </div>
                  <h3 className="font-heading font-extrabold text-lg text-foreground">John Boateng</h3>
                  <p className="text-xs font-semibold text-amber-600 dark:text-amber-400">Chief Executive Officer (CEO)</p>
                  <p className="text-[11px] text-muted-foreground mt-2 leading-relaxed">
                    Sets company strategy, approves major investments, builds partnerships & drives overall business growth.
                  </p>
                </div>
                {/* Connecting Line Down */}
                <div className="w-0.5 h-8 bg-brand-400/60" />
                <div className="w-[82%] h-0.5 bg-brand-400/60 relative">
                  <div className="absolute left-0 top-0 w-0.5 h-6 bg-brand-400/60" />
                  <div className="absolute left-1/2 -translate-x-1/2 top-0 w-0.5 h-6 bg-brand-400/60" />
                  <div className="absolute right-0 top-0 w-0.5 h-6 bg-brand-400/60" />
                </div>
              </div>

              {/* Level 2: Functional Division Managers */}
              <div className="grid grid-cols-3 gap-6 w-full pt-4">
                {/* Division 1: Operations & Logistics */}
                <div className="flex flex-col items-center space-y-6">
                  <div className="p-4 rounded-2xl bg-blue-500/10 border-2 border-blue-500/40 shadow-md text-center w-full">
                    <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded bg-blue-600 text-white mb-2 inline-block">
                      Operations & Logistics
                    </span>
                    <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 text-white font-bold text-sm flex items-center justify-center mx-auto mb-1 shadow-xs">
                      PK
                    </div>
                  ...
                  {/* Division Staff Nodes */}
                  <div className="space-y-3 w-full">
                    {[
                      { name: "Richard Adanu", title: "Shipping & Documentation Officer", icon: FileText, num: "07" },
                      { name: "Isaac Ohene Nyanteh", title: "Car Rental & Fleet Coordinator", icon: Briefcase, num: "08" },
                      { name: "Stanley Ofori Agyeman", title: "Entertainment & Customer Experience Officer", icon: Headphones, num: "09" },
                      { name: "Leslie Nii Okai", title: "Facility Manager", icon: Wrench, num: "10" },
                      { name: "Isaac Kumah", title: "Facility Manager", icon: Wrench, num: "11" },
                    ].map((staff) => (
                      <div
                        key={staff.name}
                        className="p-3 rounded-xl bg-surface-secondary border border-border/50 text-left flex items-center gap-3 shadow-xs hover:border-blue-400/50 transition-all group"
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          <div className="font-bold text-xs text-foreground truncate">{staff.name}</div>
                          <div className="text-[10px] text-muted-foreground truncate">{staff.title}</div>
                        </div>
                      </div>
                    ))}
                  </div>
                ...
                {/* Division 2: Sales & Business Dev */}
                <div className="flex flex-col items-center space-y-6">
                  <div className="p-4 rounded-2xl bg-emerald-500/10 border-2 border-emerald-500/40 shadow-md text-center w-full">
                    <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded bg-emerald-600 text-white mb-2 inline-block">
                      Sales & Business Dev
                    </span>
                    <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-emerald-500 to-teal-700 text-white font-bold text-sm flex items-center justify-center mx-auto mb-1 shadow-xs">
                      SA
                    </div>
                  ...
                  <div className="space-y-3 w-full">
                    {[
                      { name: "Richmond Ganaku", title: "Vehicle Sales Executive", icon: Car, num: "05" },
                      { name: "Reynold Antwi", title: "Marketing & Customer Relations Officer", icon: Users, num: "06" },
                    ].map((staff) => (
                      <div
                        key={staff.name}
                        className="p-3 rounded-xl bg-surface-secondary border border-border/50 text-left flex items-center gap-3 shadow-xs hover:border-emerald-400/50 transition-all group"
                      >
                ...
                {/* Division 3: Finance & Admin */}
                <div className="flex flex-col items-center space-y-6">
                  <div className="p-4 rounded-2xl bg-purple-500/10 border-2 border-purple-500/40 shadow-md text-center w-full">
                    <span className="text-[10px] font-bold uppercase tracking-wider px-2.5 py-0.5 rounded bg-purple-600 text-white mb-2 inline-block">
                      Finance & Administration
                    </span>
                    <div className="w-11 h-11 rounded-xl bg-gradient-to-br from-purple-500 to-purple-700 text-white font-bold text-sm flex items-center justify-center mx-auto mb-1 shadow-xs">
                      LA
                    </div>
                  ...
                  <div className="w-0.5 h-6 bg-purple-400/50" />
                  
                  <div className="p-4 rounded-xl bg-surface-secondary border border-purple-500/30 text-center w-full text-xs text-muted-foreground space-y-2">
                    <div className="font-bold text-foreground text-xs">Direct CEO Reporting</div>
                    <p className="text-[11px] leading-relaxed">
                      Manages accounting, payroll, budgeting, invoicing, expense tracking, office administration, and financial reporting.
                    </p>
                  </div>
                </div>
              </div>
      ...
      {/* VIEW 3: Table View (Exact docx match) */}
      {viewMode === "table" && (
        <section className="space-y-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Structured overview matching the AnyCargh organizational document table.
              </p>
            </div>
          </div>

          <div className="rounded-2xl border border-border/60 bg-background overflow-hidden shadow-sm overflow-x-auto">
              ...
              <tbody className="divide-y divide-border/40 text-foreground">
                {filteredMembers.map((member) => (
                  <tr key={member.id} className="hover:bg-surface-secondary/50 transition-colors">
                    <td className="py-4 px-4 font-bold text-center text-muted-foreground text-xs">
                      {member.number}
                    </td>
      ...
      {/* VIEW 4: Directory Grid Profile Cards */}
      {(viewMode === "grid" || viewMode === "chart" || viewMode === "diagram") && (
        <div className="space-y-6 pt-4 border-t border-border/50">
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            <div>
              ...
              <p className="text-xs text-muted-foreground">
                Filter by department or search to review detailed member roles.
              </p>
            </div>
          </div>
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="flex items-start justify-between gap-3">
                        <div className="flex items-center gap-3">
                          <div
                            className={cn(
                              "w-12 h-12 rounded-2xl bg-gradient-to-br text-white font-bold text-base flex items-center justify-center shadow-md shrink-0 border border-white/20",
                              member.color
                            )}
                          >
                            {member.initials}
      ...
      {/* Explicit Reporting Lines Breakdown Card */}
      <section className="space-y-6 pt-6 border-t border-border/50">
        <div className="text-center max-w-xl mx-auto space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            Official departmental reporting alignment specified in the corporate document.
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="p-6 rounded-2xl border-blue-500/30 bg-gradient-to-br from-blue-500/5 to-transparent space-y-4">
            <div className="flex items-center gap-2 text-blue-600 dark:text-blue-400 font-bold text-sm font-heading">
            ...
            <ul className="space-y-2.5 text-xs text-muted-foreground">
              <li className="flex items-start gap-2">
                ...
                <span>Shipping & Documentation Officer</span>
              </li>
              <li className="flex items-start gap-2">
                <CheckCircle2 className="w-3.5 h-3.5 text-blue-500 shrink-0 mt-0.5" />
                <span>Car Rental & Fleet Coordinator</span>
              </li>
              <li className="flex items-start gap-2">
                <CheckCircle2 className="w-3.5 h-3.5 text-blue-500 shrink-0 mt-0.5" />
                <span>Facility Managers (2 Members)</span>
              </li>
              <li className="flex items-start gap-2">
                <CheckCircle2 className="w-3.5 h-3.5 text-blue-500 shrink-0 mt-0.5" />
                <span>Entertainment & Customer Experience Officer</span>
              </li>
            </ul>
          </Card>

          <Card className="p-6 rounded-2xl border-emerald-500/30 bg-gradient-to-br from-emerald-500/5 to-transparent space-y-4">
            ...
            <ul className="space-y-2.5 text-xs text-muted-foreground">
              <li className="flex items-start gap-2">
                ...
                <span>Vehicle Sales Executive</span>
              </li>
              <li className="flex items-start gap-2">
                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-500 shrink-0 mt-0.5" />
                <span>Marketing & Customer Relations Officer</span>
              </li>
            </ul>
          </Card>

          <Card className="p-6 rounded-2xl border-purple-500/30 bg-gradient-to-br from-purple-500/5 to-transparent space-y-4">
            <div className="flex items-center gap-2 text-purple-600 dark:text-purple-400 font-bold text-sm font-heading">
              <Calculator className="w-4 h-4" />
              Finance & Administration
            </div>
            <p className="text-xs text-muted-foreground leading-relaxed">
              Reports directly to the Chief Executive Officer (CEO); no direct reports under this division.
            </p>
          </Card>
        </div>
      </section>

      {/* Lightbox Modal for Full Screen Diagram View */}
      {lightboxOpen && (
        <div
          className="fixed inset-0 z-50 bg-black/90 backdrop-blur-md flex items-center justify-center p-4"
          onClick={() => setLightboxOpen(false)}
        >
          <div className="relative max-w-7xl w-full max-h-[90vh] overflow-auto bg-white rounded-2xl p-4 space-y-4">
            <div className="flex items-center justify-between border-b pb-3 text-black">
              <h3 className="font-bold text-sm">AnyCargh Official Organizational Chart Diagram</h3>
              <Button
                variant="outline"
                size="sm"
                onClick={() => setLightboxOpen(false)}
